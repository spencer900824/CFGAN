...
def UseInfoPreprocessing(UseInfo):
    ...
    UseInfo.drop(['merchantId'], axis=1, inplace=True)
    UseInfo.drop(['category'], axis=1, inplace=True)
    ...
    UseInfo['useAge_scaled'] = scaler.fit_transform(
        UseInfo['useAge'].values.reshape(-1, 1))
...
def select_negative_items(realData, num_pm, num_zr):
    ...
    for i in range(data.shape[0]):
        ...
        all_item_index = random.sample(range(data.shape[1]), NUM_ITEM)
...
def main(userCount, itemCount, testSet, trainVector, trainMaskVector,
    ...
    UseInfo_pre = np.insert(UseInfo_pre, 0, [0, 0, 0, 0], axis=0)
    ...
    for epoch in range(epochCount):
        ...
        for step in range(G_step):
            ...
            realData = Variable(copy.deepcopy(
                trainVector[leftIndex:leftIndex + batchSize_G]))
            eu = Variable(copy.deepcopy(
                trainVector[leftIndex:leftIndex + batchSize_G]))
            useInfo_batch = Variable(copy.deepcopy(
                UseInfo_pre[leftIndex:leftIndex + batchSize_G]))
            # print("UseInfo: ", useInfo_batch)
            ...
            realData_zp = Variable(torch.ones_like(
                realData)) * eu + Variable(torch.zeros_like(realData)) * ku_zp
            ...
            g_loss = np.mean(np.log(1.-fakeData_result.detach().numpy()+10e-5)
        ...
        for step in range(D_step):
            ...
            realData = Variable(copy.deepcopy(
                trainVector[leftIndex:leftIndex+batchSize_D]))
            eu = Variable(copy.deepcopy(
                trainVector[leftIndex:leftIndex + batchSize_G]))
            useInfo_batch = Variable(copy.deepcopy(
                UseInfo_pre[leftIndex:leftIndex + batchSize_G]))
        ...
        if(epoch % 1 == 0):
            ...
            for testUser in testSet.keys():
                ...
                useInfo_index = Variable(copy.deepcopy(
                    torch.tensor(np.expand_dims(UseInfo_pre[index], axis=0))))
                #  Exclude the purchased vector that have occurred in the training set
                result = G(data.reshape(1, NUM_ITEM), useInfo_index) + \
                    Variable(copy.deepcopy(trainMaskVector[index]))
                result = result.reshape(NUM_ITEM)
...
def result_plt(result_precision):
    ...
    plt.xlabel('epoch')
    plt.plot(result_precision[:, 0],
             result_precision[:, 1], "r-*", label='precision')
    plt.ylim([0, 0.6])
    plt.legend()
    plt.show()
...
if __name__ == '__main__':
    ...
    file_name, NUM_ITEM = data.pre_process_raw_data(
        input_file='raw.data', output_file='welldone')

    UseInfo = data.loadUseInfo(f"{file_name}/welldone.user", "|")
...
#    ItemInfo = data.loadItemInfo("data/ml-100k/u.item"  , "|")
    ...
    trainSet, train_use, train_item = data.loadTrainingData(
        f"{file_name}/u/u1.base", ",")
    testSet, test_use, test_item = data.loadTestData(
        f"{file_name}/u/u1.base", ",")
